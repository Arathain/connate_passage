...
import arathain.miku_machines.content.block.*;
import arathain.miku_machines.content.block.entity.WorldshellBlockEntity;
...
public interface ConnateBlocks {
	...
	static <T extends Block> T register(String id, T block, boolean createItem) {
		Identifier identity = new Identifier(MikuMachines.MODID, id);
	...
	private static <T extends BlockEntity> BlockEntityType<T> register(String name, BlockEntityType<T> type) {
		BLOCK_ENTITY_TYPES.put(new Identifier(MikuMachines.MODID, name), type);
		return type;
	}
